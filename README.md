...
**KernReg** allows for the estimation of a regression function as well as their derivatives. The degree of the polynomial must be equal to ```degree = derivative + 1```
```derivative + 3```, ```derivative + 5```, or ```derivative + 7```.
